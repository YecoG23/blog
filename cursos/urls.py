...
urlpatterns = [
    # urls for cursos
    path('lista', CursoListView.as_view(), name='list_cursos'),
    re_path(r'^add$', CursoCreateView.as_view(), name='new_curso'),
    path('<slug:slug>/detail', CursoDetailView.as_view(), name='view_curso'),
    ...
    path('<slug:slug>/aula/<int:pk>/detail', AulaDetailView.as_view(), name='view_aula'),
